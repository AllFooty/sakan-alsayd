  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
        <Info size={18} className="text-amber-700 mt-0.5 shrink-0" aria-hidden />
      ...
      <div className="rounded-xl border border-gray-200 bg-white px-4 py-3">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs">
          <span className="text-gray-500 font-medium">{t('legend.title')}</span>
          <span className="inline-flex items-center gap-2">
            <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-50 text-emerald-700">
              <Check size={12} strokeWidth={2.5} aria-hidden />
            </span>
            <span className="text-gray-700">{tLegend('full')}</span>
          </span>
          <span className="inline-flex items-center gap-2">
            <span className="relative inline-flex items-center justify-center w-5 h-5 rounded-full bg-amber-50 text-amber-700">
              <Check size={12} strokeWidth={2.5} aria-hidden />
              <span className="absolute -bottom-0.5 end-0.5 w-1 h-1 rounded-full bg-amber-500" aria-hidden />
            </span>
            <span className="text-gray-700">{tLegend('scoped')}</span>
          </span>
          <span className="inline-flex items-center gap-2">
            <span className="inline-flex items-center justify-center w-5 h-5 text-gray-400">
              <Minus size={12} aria-hidden />
            </span>
            <span className="text-gray-700">{tLegend('none')}</span>
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full ps-10 pe-10 py-2.5 text-sm rounded-xl border border-gray-300 bg-white focus:outline-none focus:ring-2 focus:ring-coral/40 focus:border-coral"
          />
          {search && (
            <button
              type="button"
              onClick={() => setSearch('')}
              className="absolute top-1/2 -translate-y-1/2 end-2 p-1 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100"
